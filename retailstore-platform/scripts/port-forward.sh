#!/bin/bash
# port-forward.sh — Forward local k8s infra ports to localhost for IntelliJ debugging
#
# After running this, IntelliJ services use SPRING_PROFILES_ACTIVE=local
# and connect to Docker Desktop k8s infra via localhost.
#
...
start_forwards() {
  ...
  echo "  Run IntelliJ service with SPRING_PROFILES_ACTIVE=local"
